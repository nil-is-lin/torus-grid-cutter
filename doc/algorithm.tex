...
\item “在切割线上”的容差为 \texttt{MERGE\_EPS = 1e-4}，\textbf{不是} $10^{-8}$；knot 曲线另用
  与面无关的顶点吸附容差 \texttt{KNOT\_SNAP\_UV = 1e-3}（$\phi$ 空间乘 $\sqrt{1+k^2}$）；
...
Knot 曲线 $v = k\,u$ 在参数方域内有 $k$ 条分支（$v = k\,u + 2\pi n$）。支持两条 knot（$k_1, k_2$）独立开关。
每个面须切开其跨越的\textbf{全部}分支，分支范围由 \texttt{knot\_branch\_range} 两端取 $\lfloor\cdot\rfloor$ 给出。
两条 $(1,k_1)$、$(1,k_2)$ 曲线把环面分成 $|k_2-k_1|$ 个区域。详见 \texttt{knot\_algorithm.tex}。
...
\begin{enumerate}
\item \texttt{normalize\_uv(val, min, max)} $=\dfrac{val-min}{max-min}\cdot 2\pi$：把任意 UV 域 $[min,max]$ 归一化到 $[0,2\pi)$，使 \texttt{unwrap\_angle}（周期 $2\pi$）能正确工作。
...
Knot 模式\textbf{不}走上述几何分段，而用解析拓扑不变量
\texttt{assign\_connected\_components\_knot}：逐面按
$L=(\lfloor(v-k_1u)/2\pi\rfloor-\lfloor(v-k_2u)/2\pi\rfloor)\bmod|k_2-k_1|$
指派区域，区域数恒为 $|k_2-k_1|$（单条曲线为 1）。详见 \texttt{knot\_algorithm.tex}。
